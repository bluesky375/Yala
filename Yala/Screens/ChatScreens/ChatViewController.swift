...
class ChatViewController: QMChatViewController, QMChatServiceDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, QMChatAttachmentServiceDelegate, QMChatConnectionDelegate, QMChatCellDelegate, QMDeferredQueueManagerDelegate, QMPlaceHolderTextViewPasteDelegate {
    ...
    class func fromStoryboard() -> ChatViewController {
        let viewController = UIStoryboard (name: "Main", bundle: nil).instantiateViewController(withIdentifier: String(describing: ChatViewController.self)) as! ChatViewController
    ...
    func updateTitle() {
        ...
        else if let recipient = ServicesManager.instance().usersService.usersMemoryStorage.user(withID: UInt(self.dialog!.recipientID)) {
                
                if recipient.fullName != nil {
                    self.title = recipient.fullName!
                } else if recipient.email != nil {
                    self.title = recipient.email!
    ...
    override func didPickAttachmentImage(_ image: UIImage!) {
        ...
        DispatchQueue.global().async { [weak self] () -> Void in
            ...
            if strongSelf.imagePickerViewController.sourceType == UIImagePickerControllerSourceType.camera {
                // newImage = newImage.fixOrientation() //AP:
            }
    ...
    func sendMessage(message: QBChatMessage) {
        ...
        // Sending message.
        ServicesManager.instance().chatService.send(message, toDialogID: self.dialog.id!, saveToHistory: true, saveToStorage: true) { [weak self] (error) ->
            Void in
            
            if let occupants = self?.dialog.occupantIDs, let senderId = self?.senderID {
                var users = ""
                for user in occupants {
                    if user.intValue != senderId {
                        users.append(String(user.intValue))
                        users.append(",")
                    }
                }
                let user = ServicesManager.instance().usersService.usersMemoryStorage.user(withID: senderId)
                var pushMessage = ""
                
                if user?.fullName != nil {
                    pushMessage = user!.fullName!
                    pushMessage.append(":")
                } else if user?.email != nil {
                    pushMessage = user!.email!
                    pushMessage.append(":")
                ...
                pushMessage.append(message.text ?? "")
                
                QBRequest.sendPush(withText: pushMessage, toUsers: users, successBlock: nil, errorBlock: nil)
            }
    ...
    override func attributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString? {
        ...
        var textColor = messageItem.senderID == self.senderID ? UIColor.white : UIColor.darkGray
        ...
        attributes[NSAttributedStringKey.font] = UIFont(name: "Helvetica", size: 15)
    ...
    override func topLabelAttributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString? {
        ...
        attributes[NSAttributedStringKey.font] = UIFont(name: "Helvetica", size: 16)
    ...
    override func bottomLabelAttributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString! {
        ...
        let textColor = messageItem.senderID == self.senderID ? UIColor.white : UIColor.darkGray
    ...
    override func collectionView(_ collectionView: QMChatCollectionView!, configureCell cell: UICollectionViewCell!, for indexPath: IndexPath!) {
        ...
        if let attachmentCell = cell as? QMChatAttachmentCell {
            ...
            if attachmentCell is QMChatAttachmentIncomingCell {
                ...
                chatCell.containerView?.bgColor = YalaThemeService.themeColor()
            }
